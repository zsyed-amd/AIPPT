 ...
 * Pair with the CONTENT_MASTER eyebrow rect (which draws the visual dash element).
 * Call after any slide builder to annotate a slide's section context.
 * @param {Object} slide - pptxgenjs slide
 * @param {string} label - Section label (e.g. "PLATFORM OVERVIEW")
 * @param {{ colors, fonts, slide: { margin } }} theme
 */
export function addEyebrowText(slide, label, theme) {
  const M = theme.slide.margin;
  const C = theme.colors;
  slide.addText(label.toUpperCase(), {
    x: M + 1.35, y: 0.16, w: 8.0, h: 0.14,
    fontSize: 8, fontFace: theme.fonts.body,
    color: C.accent, charSpacing: 6, bold: false,
  });
}
